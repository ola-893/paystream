# Implementation Plan: Morph to PayStream Refactor

## Overview

Systematically remove all "Morph" references from the PayStream project by renaming files and updating content. Changes are ordered to minimize breakage: contracts first, then tests, docs, frontend, and finally verification.

## Tasks

- [x] 1. Smart Contract Refactoring
  - [x] 1.1 Rename and update MorphStream.sol to PayStreamStream.sol
    - Rename file from `contracts/MorphStream.sol` to `contracts/PayStreamStream.sol`
    - Update contract name from `MorphStream` to `PayStreamStream`
    - Update @title and @dev NatSpec comments
    - Replace all "MorphStream:" error prefixes with "PayStreamStream:"
  ...
  - [x] 1.2 Clean and recompile contracts
    ...
    - Verify `artifacts/contracts/PayStreamStream.sol/` exists
    - _Requirements: 5.1, 5.2_

- [x] 2. Test File Refactoring
  - [x] 2.1 Rename and update test file
    - Rename `test/MorphStream.test.js` to `test/PayStreamStream.test.js`
    - Update all `MorphStream` references to `PayStreamStream` in test code
    - Update variable names from `morphStream` to `payStreamStream`
    - _Requirements: 2.1, 2.2_

  - [x] 2.2 Update package.json test script
    - Change test script from `MorphStream.test.js` to `PayStreamStream.test.js`
    - _Requirements: 2.3_
...
- [x] 3. Documentation Refactoring
  - [x] 3.1 Rename morphstream.md to paystreamstream.md
    - Rename `docs/contracts/morphstream.md` to `docs/contracts/paystreamstream.md`
    - Update all content within the file to reference PayStreamStream
    - Update variable names in code examples from `morphStream` to `payStreamStream`
    - _Requirements: 3.1, 3.3, 3.4_

  - [x] 3.2 Update docs/SUMMARY.md navigation
    - Change link from `[MorphStream](contracts/morphstream.md)` to `[PayStreamStream](contracts/paystreamstream.md)`
    - _Requirements: 3.2_
  ...
  - [x] 3.4 Update root documentation files
    - Update `README.md` - replace all MorphStream references with PayStreamStream
    - Update `DEPLOYMENT.md` - replace all MorphStream references with PayStreamStream
    - Remove any "Morph Holesky" or "Morph network" references
    - _Requirements: 3.3, 3.5_

- [x] 4. Frontend Refactoring
  - [x] 4.1 Update Docs.jsx page
    - Replace all `MorphStream` with `PayStreamStream` in content
    - Update `morphstream-contract` key to `paystreamstream-contract`
    - Update navigation sidebar item from "MorphStream" to "PayStreamStream"
  ...
  - [x] 4.3 Update contactInfo.js comment
    - Change comment from "MorphStream on Sepolia" to "PayStreamStream on Sepolia"
    - _Requirements: 6.3_

  - [x] 4.4 Update vite-project/README.md
    - Change "MorphStream Frontend" to "PayStream Frontend"
    - _Requirements: 6.4_

- [x] 5. Demo and Server Refactoring
  - [x] 5.1 Update demo files
    - Update `demo/consumer.ts` - change `MORPHSTREAM_ADDRESS` to `PAYSTREAMSTREAM_ADDRESS`
  ...
  - [x] 5.2 Update server middleware
    - Update JSDoc comment in `server/middleware/payStreamMiddleware.js`
    - Change "MorphStream Contract Address" to "PayStreamStream Contract Address"
    - _Requirements: 4.2_

- [x] 6. Configuration and Deploy Script Updates
  - [x] 6.1 Update deploy script
    - Update `scripts/deploy.js` to reference PayStreamStream
