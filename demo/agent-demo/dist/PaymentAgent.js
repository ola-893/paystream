...
const x402Protocol_1 = require("./x402Protocol");
...
class PaymentAgent {
    constructor(config) {
        // State tracking
        this._mneeBalance = 0n;
        this._dailySpent = 0n;
        this._activeStreams = new Map();
        this._requestCount = 0;
        this._paymentCount = 0;
        this._lastDailyReset = Date.now();
        // Initialization flag
        this._initialized = false;
        // Dry-run mode tracking (Requirements: 9.3, 9.7)
        this._dryRun = false;
        this._mockStreamCounter = 0;
        this._mockTxCounter = 0;
        this.config = config;
        ...
        this.mneeContract = new ethers_1.Contract(config.mneeToken, ERC20_ABI, this.wallet);
        this.flowPayContract = new ethers_1.Contract(config.flowPayContract, FLOWPAY_ABI, this.wallet);
    }
    /**
    ...
    async initialize() {
        if (this._dryRun) {
            ...
            this._mneeBalance = ethers_1.ethers.parseEther('100');
        }
        else {
    ...
    checkBudget(amount) {
        ...
        if (this._dailySpent + amount > this.config.dailyBudget) {
            const remaining = this.config.dailyBudget - this._dailySpent;
            throw new Error(`Budget exceeded: daily limit is ${ethers_1.ethers.formatEther(this.config.dailyBudget)} MNEE, ` +
                `already spent ${ethers_1.ethers.formatEther(this._dailySpent)} MNEE, ` +
                `remaining ${ethers_1.ethers.formatEther(remaining)} MNEE, ` +
                `requested ${ethers_1.ethers.formatEther(amount)} MNEE`);
        }
    ...
    checkBalance(amount) {
        if (amount > this._mneeBalance) {
            ...
            throw new Error(`Insufficient MNEE balance: need ${ethers_1.ethers.formatEther(amount)} MNEE, ` +
                `have ${ethers_1.ethers.formatEther(this._mneeBalance)} MNEE, ` +
                `shortfall ${ethers_1.ethers.formatEther(shortfall)} MNEE`);
        }
    }
    /**
     ...
     */
    async createStream(requirement, serviceUrl, purpose = 'API access') {
        try {
            ...
            if (requirement.mode === 'streaming') {
                ...
                const rate = ethers_1.ethers.parseEther(requirement.rate || '0.0001');
                amount = requirement.minDeposit
                    ? ethers_1.ethers.parseEther(requirement.minDeposit)
                    : rate * BigInt(duration);
            }
            else {
                // For per-request mode, use the specified amount
                amount = ethers_1.ethers.parseEther(requirement.amount || '0.01');
            ...
            if (!this.hasSufficientBalance(amount)) {
                ...
                return {
                    success: false,
                    error: `Insufficient MNEE balance: need ${ethers_1.ethers.formatEther(amount)} MNEE, ` +
                        `have ${ethers_1.ethers.formatEther(this._mneeBalance)} MNEE, ` +
                        `shortfall ${ethers_1.ethers.formatEther(shortfall)} MNEE`,
                };
            }
            ...
            if (this._dryRun) {
                ...
                const streamInfo = {
                    ...
                    amount,
                    rate: requirement.rate ? ethers_1.ethers.parseEther(requirement.rate) : 0n,
                    startTime,
                    expiresAt,
                };
            ...
            const currentAllowance = await this.mneeContract.allowance(this.wallet.address, this.config.flowPayContract);
            if (currentAllowance < amount) {
                const approveTx = await this.mneeContract.approve(this.config.flowPayContract, amount);
            ...
            const createTx = await this.flowPayContract.createStream(requirement.recipient, duration, amount, metadataString);
     ...
     */
    extractStreamIdFromReceipt(receipt) {
    ...
    async fetch(url, options = {}) {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                if ((0, x402Protocol_1.isX402Response)(response.status, responseHeaders)) {
                    // Parse x402 requirements
                    const requirement = (0, x402Protocol_1.parseX402Headers)(responseHeaders);
                    if (!requirement) {
                        return {
                            success: false,
