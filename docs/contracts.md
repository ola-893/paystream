...
PayStreamStream enables continuous payment streams using TCRO tokens. Funds flow from sender to recipient over time, calculated per-second.
...
| amount | uint256 | Total TCRO amount (in wei) |
...
- `duration > 0`
- `recipient != address(0)`
- Sender must have sufficient TCRO balance

**Example:**
```javascript
// Create stream with TCRO
await payStreamStream.createStream(
  recipientAddress,
  3600, // 1 hour
  ethers.parseEther('10'), // 10 TCRO
  '{"purpose": "API access"}',
  { value: ethers.parseEther('10') } // Send TCRO with transaction
);
```
...
struct Stream {
    ...
    uint256 totalAmount;   // Total TCRO locked
    uint256 flowRate;      // TCRO per second
...
1. **No Token Approval Required**: Users send TCRO directly with stream creation
...
| totalAmount | No | Total TCRO locked |
...
```javascript
{
  ...
  args: {
    ...
    sender: '0x1234...',
    recipient: '0x5678...',
    totalAmount: 10000000000000000000n, // 10 TCRO
...
contract.on('Withdrawn', (streamId, recipient, amount) => {
  ...
  console.log(`${ethers.formatEther(amount)} TCRO to ${recipient}`);
});
