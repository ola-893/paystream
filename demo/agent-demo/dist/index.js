...
const cli = new CLIOutput_1.CLIOutput(cliConfig);
/**
...
function validateEnvironmentOrExit() {
    ...
    if (!result.valid || !result.config) {
        ...
        console.error(chalk_1.default.red((0, config_1.formatValidationError)(result)));
        // Display each missing variable clearly
        if (result.missingVariables.length > 0) {
            console.error(chalk_1.default.yellow('\nRequired variables:'));
            for (const varName of result.missingVariables) {
                console.error(chalk_1.default.yellow(`  ✗ ${varName}`));
            }
        }
...
async function main() {
    ...
    if (options.check) {
        ...
        const agentConfig = {
            ...
            dailyBudget: ethers_1.ethers.parseEther(config.DAILY_BUDGET),
            flowPayContract: config.FLOWPAY_CONTRACT,
            mneeToken: config.MNEE_TOKEN,
            geminiApiKey: config.GEMINI_API_KEY,
            dryRun: options.dryRun || false,
        };
        const agent = new PaymentAgent_1.PaymentAgent(agentConfig);
        await agent.initialize();
        const runner = new DemoRunner_1.DemoRunner(agent, cli, config.SERVER_URL);
    ...
    try {
        ...
        const agentInfoLines = [
            ...
            `MNEE Balance: ${ethers_1.ethers.formatEther(state.mneeBalance)} MNEE`,
            `Daily Budget: ${ethers_1.ethers.formatEther(agent.dailyBudget)} MNEE`,
            `Remaining Budget: ${ethers_1.ethers.formatEther(agent.getRemainingBudget())} MNEE`,
        ];
        ...
        if (options.verbose) {
            ...
            cli.debug(`  MNEE Balance: ${ethers_1.ethers.formatEther(state.mneeBalance)} MNEE`);
            cli.debug(`  Daily Spent: ${ethers_1.ethers.formatEther(state.dailySpent)} MNEE`);
        ...
        if (options.verbose) {
            ...
            const testAmount = ethers_1.ethers.parseEther('0.001');
            ...
            const hugeAmount = ethers_1.ethers.parseEther('1000000');
        ...
        if (options.verbose) {
            ...
            const testStream = {
                ...
                amount: ethers_1.ethers.parseEther('1'),
                rate: ethers_1.ethers.parseEther('0.0001'),
